...
`tests/gdscript_suites.json`. By default it evaluates the corpus in `--shards`
Godot processes, each taking a contiguous slice of the ordered population, and
refuses any run whose shards do not tile that population or whose completion
evidence does not account for every case the ledger declares. `--execution
isolated` gives each case its own process instead, which is how a case that
crashes or hangs the engine is pinpointed; `--case` and `--jobs` belong to that
path. CI runs four shards, one per vCPU on the GitHub-hosted Linux runner. `project/tests/corpus_runner.gd`
...
python3 scripts/run_corpus_triage.py --godot /path/to/godot --corpus res://tests/corpus/analyzer --report /tmp/analyzer-results.json --shards 4
```
...
`--order reverse` orders the whole population before slicing it, so it changes both the
sequence within a shard and which cases share a process; a case whose outcome changes
under it is a contamination bug, not a corpus result.
