...
namespace {
Dictionary corpus_result_to_dictionary(const CorpusResult &p_result) {
	Dictionary dictionary;
	dictionary["output"] = p_result.output;
	dictionary["ok"] = p_result.ok;
	dictionary["analysis_ran"] = p_result.analysis_ran;
	dictionary["infrastructure_error"] = p_result.infrastructure_error;
	if (p_result.fixture_index_reported) {
		Dictionary fixture_index;
		fixture_index["sources"] = p_result.fixture_sources;
		fixture_index["heads"] = p_result.fixture_heads;
		fixture_index["annotation_providers"] = p_result.fixture_annotation_providers;
		fixture_index["conformance_providers"] = p_result.fixture_conformance_providers;
		dictionary["fixture_index"] = fixture_index;
	}
	return dictionary;
}
} // namespace
...
Dictionary BaristaScriptAnalyzerProbe::evaluate_corpus(const PackedByteArray &p_bytes, const String &p_path, const String &p_stage, const PackedStringArray &p_fixture_paths) const {
	return corpus_result_to_dictionary(evaluate_corpus_case(p_bytes, p_path, p_stage, p_fixture_paths));
}

Dictionary BaristaScriptAnalyzerProbe::corpus_format_controls() const {
	Dictionary results;
	BSParser parser;
	results["failure_without_diagnostic"] = corpus_result_to_dictionary(format_corpus_result(parser, ERR_BUG, true));
	...
	results["ordered_errors"] = corpus_result_to_dictionary(format_corpus_result(parser, ERR_PARSE_ERROR, true));
	errors.clear();
	...
	results["ordered_warnings"] = corpus_result_to_dictionary(format_corpus_result(parser, OK, true));
	BSParser::ParserError error;
	error.line = 1;
	error.column = 1;
	error.message = "synthetic transport error";
	errors.push_back(error);
	results["errors_suppress_warnings"] = corpus_result_to_dictionary(format_corpus_result(parser, ERR_PARSE_ERROR, true));
	return results;
}
