...
## The root this runner walks when no --corpus is given.
##
...
## scripts/run_corpus_triage.py, which runs its whole population in sharded processes
## and holds it to a pinned residual-failure set; collecting it here would make the aggregate
